...
TODO:
- Old to new
    -  see why resulting pre/post conversion cols lists are different lengths
- New to old
    - Add functionality to convert new to old data format

Sample usage (assuming new_data_format_sample is the new data format sample):
>>> old_data = pd.read_csv("path_to_old_data.csv", header=1)
>>> old_to_new_df = convert_old_to_new_final_fixed(
    old_data, format_dictionary_updated)
'''
...
# Import data
old_data = pd.read_csv('Example Old Data/old_data_format.csv', header=1)
new_data = pd.read_csv('Example Old Data/new_data_format.csv')
...
new_cols = ['sample.id', 'lab.id.x', 'method', 'cas.number', 'analyte', 'result',
...
def clean_old_data(old_df):
    ...
    # identify nan values and fill over total dataframe
        # if any cell contains NA, U, ND, nan, NAN, N/A, or is blank, replace with nan
...
def convert_old_to_new_final_fixed(old_df, format_dict, output_csv=False):
    """
    ...
    'method': ['M4500 NH3 D - TKN_W 4500NH3 D', 'E300 - 300_W', 'E300 - 300_W', 'SM4500P E - P-Ortho M4500P E',
               'SM4500P E - P_TW M4500P E', 'E160.1 - TDS_W_160.1', 'E120.1 - COND_W', 'SW9040C - pH_W_9040C',
               'SW9040C - pH_W_9040C', 'E160.2 - TSS_W_160.2', '200.8'],
    'cas.number': ['7727-37-9TKN', '14797-55-8', '14797-65-0', '7723-14-0', '7723-14-0', 'TDS', 'COND', 'PH', 'TEMP', 'TSS', 'SE'],
    'analyte': ['Nitrogen, Total Kjeldahl', 'Nitrogen, Nitrate (As N)', 'Nitrogen, Nitrite  (As N)', 
                'Phosphorus, Total Orthophosphate (as P)', 'Phosphorus, Total (As P)', 'Total Dissolved Solids (Residue, Filterable)',
                'EC', 'pH', 'Temp Deg C @pH', 'TSS', 'Selenium, Total'],
    'mdl': [0.1, 0.15, 0.15, 0.01, 0.02, 5, 5, 0.1, 0, 2.5, 0.2],
    'rl': [0.5, 0.5, 0.5, 0.05, 0.05, 10, 5, 0.1, 0, 2.5, 0.2] #note RL was not reported by ALS for selenium, just MDL
    })
    
    # Extract analytes from the format dictionary
    analyte_cols = [key for key in format_dict.keys() if key not in ['Location', 'Date', 'Irr/Storm', 'Station', 'Dup', 'ID']]
    
    # Melt the old dataframe to convert analyte columns to rows
    melted_data = old_df.melt(id_vars=[key for key in format_dict.keys() if key not in analyte_cols],
    ...
    melted_data['analyte'] = melted_data['analyte'].apply(lambda x: format_dict[x][0] if isinstance(format_dict[x], list) else format_dict[x])
    ...
    melted_data.loc[melted_data['analyte'] == 'Selenium, Total', 'units'] = "ug/L"
    melted_data.loc[melted_data['analyte'] == 'EC', 'units'] = "umhos/cm"
    melted_data.loc[melted_data['analyte'] == 'pH', 'units'] = "pH"
    melted_data.loc[melted_data['analyte'] == 'Temp Deg C @pH', 'units'] = "Celcius"
    # add warning for selenium to double check units
    if 'Selenium, Total' in melted_data['analyte'].values:
        print("Warning: Selenium detected. Please double-check the units for Selenium.")
    ...
    if output_csv:
        merged_data.to_csv('Output/old_to_new_data.csv', index=False)
    
    return merged_data

# Diagnostic code to check the conversion in Python only ######################

def report_non_overlapping_columns(old_df, new_df, format_dict):
    """
    ...
    returns:
        non_overlapping_old: columns in old_df that are not keys in format_dict
        non_overlapping_new: columns in new_df that are not in the converted data

    Example usage:
    non_overlapping_old, non_overlapping_new = report_non_overlapping_columns(
        old_data, new_data, format_dictionary_updated)
    """
    
    # Convert the old data format using the main conversion function
    converted_data = convert_old_to_new_final(old_df, format_dict)
    
    # Identify columns in old_df that are not keys in format_dict
    non_overlapping_old = set(old_df.columns) - set(format_dict.keys())
    
    # Identify columns in new_df that are not in the converted data
    non_overlapping_new = set(new_df.columns) - set(converted_data.columns)
    
    return non_overlapping_old, non_overlapping_new
